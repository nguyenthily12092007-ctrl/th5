...
def plot_analysis(df):
    print("📈 Creating static visualizations...", flush=True)
    sns.set_theme(style="whitegrid")
    
    # 1. Biểu đồ phân bố số tiền giao dịch (Sửa sang màu hồng dâu #FF8E9E)
    print("  → Plotting transaction amount distribution...", end=" ", flush=True)
    plt.figure(figsize=(10, 6))
    sns.histplot(data=df, x="Amount", hue="Anomaly", bins=30, kde=True, palette={0: "#FF8E9E", 1: "#D32F2F"}, multiple="stack")
    plt.title("Transaction Amount Distribution", fontsize=14, fontweight="bold", color="#FF6B8B")
    plt.xlabel("Amount ($)")
    plt.tight_layout()
    ...
    # 2. Biểu đồ hộp Outliers theo loại giao dịch (Tông hồng dâu pastel)
    print("  → Plotting amount by transaction type...", end=" ", flush=True)
    plt.figure(figsize=(10, 6))
    sns.boxplot(x="TransactionType", y="Amount", hue="Anomaly", data=df, palette={0: "#FFE3E3", 1: "#FF8E9E"})
    plt.title("Amount by Transaction Type", fontsize=14, fontweight="bold", color="#FF6B8B")
    ...
    # 3. Biểu đồ cột số lượng theo ngày trong tuần (Ép sang hồng đồng bộ)
    print("  → Plotting transactions by day of week...", end=" ", flush=True)
    plt.figure(figsize=(10, 6))
    day_counts = df["DayOfWeek"].value_counts()
    sns.barplot(x=day_counts.index, y=day_counts.values, color="#FF8E9E")
    plt.title("Transactions by Day of Week", fontsize=14, fontweight="bold", color="#FF6B8B")
    ...
    plt.close()
    print("✓")
    print("✅ All plots saved successfully with Pink Pastel theme!")
